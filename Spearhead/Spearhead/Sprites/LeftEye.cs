...
{
    /// <summary>
    /// This class creates the left eye boss component
    /// </summary>
    class LeftEye : Component
    {
        #region Fields
        ContentManager Content; // The Content Manager
        #endregion

        public LeftEye(Boss boss, Vector2 origin, Vector2 offset, ContentManager content, int health, int value, bool mirrored, bool collidable, bool critical)
            : base(boss, origin, offset, "Images/level-4-boss-left-eye", content, health, value, mirrored, collidable, critical)
        {
            Texture = content.Load<Texture2D>("Images/level-4-boss-left-eye");
            FrameSize = new Vector2(Texture.Width / 2, Texture.Height);
            Damageable = true;
            Destroyed = false;
            HitArea = new Rectangle((int)Position.X + 9, (int)Position.Y + 13, 59, 37);
            ExplosionCount = 3;
            Content = content;
        }
