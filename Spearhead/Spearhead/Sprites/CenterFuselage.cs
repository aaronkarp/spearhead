...
{
    /// <summary>
    /// This class creates the center fuselage of the Rail Wing boss
    /// </summary>
    class CenterFuselage : Component
    {
        #region Fields
        ...
        public CenterFuselage(Boss boss, Vector2 origin, Vector2 offset, ContentManager content, int health, int value, bool mirrored, bool collidable, bool critical)
            : base(boss, origin, offset, "Images/level-5-boss-ship-fuselage-spritesheet", content, health, value, mirrored, collidable, critical)
        {
            Texture = content.Load<Texture2D>("Images/level-5-boss-ship-fuselage-spritesheet");
            FrameSize = new Vector2(Texture.Width / 2, Texture.Height);
            Destroyed = false;
            ExplosionCount = 3;
            fireTime = TimeSpan.FromSeconds(.35f);
            previousFireTime = TimeSpan.Zero;
            fireStopTime = TimeSpan.FromSeconds(1f);
            previousFireStopTime = TimeSpan.Zero;
            firing = true;
            projectileTexture = "Images/laser-round";
            gunOffset = new Vector2(32, 232);
            projectileVerticalSpeed = 7f;
        ...
        public override void UpdateComponent(GameTime gameTime, Level5Boss boss, string leftDoorStatus, string rightDoorStatus)
        {
            ...
            HitArea = new Rectangle((int)Position.X + 20, (int)Position.Y + 172, 30, 28);
